 ...
 */
export async function getDMEligibleUsersController(req: AuthenticatedRequest, res: Response) {
  try {
...
export async function createConversationController(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (error instanceof Error && error.message.includes("must share at least one project")) {
      return res.status(403).json({ error: "Users must share at least one project to create a DM" });
    }
 ...
 */
export async function getUserConversationsController(req: AuthenticatedRequest, res: Response) {
  try {
    const userId = req.user?.id;
 ...
 */
export async function canCreateDMController(req: AuthenticatedRequest, res: Response) {
  try {
    const userId = req.user?.id;
